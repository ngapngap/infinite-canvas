...
const adminMenus = [
  { key: "/admin/prompts", icon: <FileTextOutlined />, label: "Quản lý Prompt" },
  { key: "/admin/assets", icon: <PictureOutlined />, label: "Thư viện tài nguyên" },
...
export default function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const pageTitle = pathname.startsWith("/admin/settings") ? "Cài đặt hệ thống" : pathname.startsWith("/admin/assets") ? "Quản lý tài nguyên" : "Quản lý Prompt";
    ...
    <Layout hasSider style={{ height: "100vh", overflow: "hidden", background: antToken.colorBgLayout }}>
      <Layout.Sider width={adminLayoutStyle.siderWidth} style={{ height: "100vh", overflow: "hidden", background: antToken.colorBgContainer, borderRight: `1px solid ${antToken.colorBorder}` }}>
        <Flex align="center" gap={12} style={{ height: adminLayoutStyle.brandHeight, padding: "0 20px", borderBottom: `1px solid ${antToken.colorBorderSecondary}` }}>
          ...
          <Typography.Text strong style={{ fontSize: 18, letterSpacing: 0 }}>Ram Canvas</Typography.Text>
        </Flex>
        <Menu
          mode="inline"
        ...
        <Flex vertical gap={8} style={{ position: "absolute", bottom: 0, insetInline: 0, padding: 12, borderTop: `1px solid ${antToken.colorBorder}`, background: antToken.colorBgContainer }}>
          <Button block icon={<HomeOutlined />} href="/canvas" target="_blank" rel="noreferrer">Đi tới Canvas</Button>
          <Button block icon={<LogoutOutlined />} onClick={logout}>Đăng xuất</Button>
        </Flex>
      </Layout.Sider>
