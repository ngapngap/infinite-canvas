...
export default function AssetLibraryPage() {
  ...
  useEffect(() => {
    if (query.isError) {
      message.error(query.error instanceof Error ? query.error.message : "Không thể tải thư viện tài nguyên");
    }
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-background text-stone-800 dark:text-stone-100">
      <main className="min-h-0 flex-1 overflow-y-auto bg-background bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] px-6 py-8 [background-size:16px_16px] dark:bg-[radial-gradient(rgba(245,245,244,.16)_1px,transparent_1px)]">
        <div className="pb-8">
          <div className="mx-auto max-w-5xl text-center">
            <h1 className="text-4xl font-semibold tracking-tight text-stone-950 dark:text-stone-100">Thư viện tài nguyên</h1>
            <p className="mt-3 text-sm text-stone-500 dark:text-stone-400">Chọn tài nguyên nhóm, thêm vào tài nguyên của tôi để tiếp tục chỉnh sửa và sử dụng.</p>
          </div>
          <div className="mx-auto mt-8 w-full max-w-2xl">
            <Input size="large" className="w-full" prefix={<Search className="size-4 text-stone-400" />} value={keyword} placeholder="Tìm theo tiêu đề" onChange={(event) => { setPage(1); setKeyword(event.target.value); }} />
          </div>
          <div className="mx-auto mt-6 max-w-6xl space-y-3">
            <div className="grid gap-2 sm:grid-cols-[56px_minmax(0,1fr)] sm:items-start">
              <div className="pt-2 text-xs font-medium text-stone-500 dark:text-stone-400">Loại</div>
            ...
            <div className="grid gap-2 sm:grid-cols-[56px_minmax(0,1fr)] sm:items-start">
              <div className="pt-2 text-xs font-medium text-stone-500 dark:text-stone-400">Thẻ</div>
              <div className="flex flex-wrap gap-2">
        ...
        <div className="mx-auto flex max-w-7xl flex-col gap-5">
          ...
          {!items.length ? <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Không tìm thấy tài nguyên" className="py-20" /> : null}
      ...
      <Drawer title="Chi tiết tài nguyên" open={Boolean(selectedAsset)} size="large" onClose={() => setSelectedAsset(null)}>
        {selectedAsset ? (
          <div className="space-y-5">
            {selectedAsset.coverUrl ? <Image src={selectedAsset.coverUrl} alt={selectedAsset.title} className="rounded-lg" /> : <div className="rounded-lg border border-stone-200 bg-stone-50 p-5 text-sm leading-6 text-stone-600 dark:border-stone-800 dark:bg-stone-900 dark:text-stone-300">{selectedAsset.content || "Chưa có ảnh bìa"}</div>}
            <div>
              <Typography.Title level={4} className="!mb-2">{selectedAsset.title}</Typography.Title>
              <div className="flex flex-wrap gap-1.5">
                <Tag>{selectedAsset.type === "image" ? "Hình ảnh" : "Văn bản"}</Tag>
            ...
            <div className="flex flex-wrap gap-2">
              {selectedAsset.type === "text" ? <Button type="primary" icon={<Copy className="size-4" />} onClick={() => copyText(selectedAsset.content)}>Sao chép văn bản</Button> : null}
              {selectedAsset.type === "image" ? <Button type="primary" icon={<Copy className="size-4" />} onClick={() => copyText(selectedAsset.url)}>Sao chép liên kết</Button> : null}
              <Button icon={<FolderPlus className="size-4" />} onClick={() => void saveToMyAssets(selectedAsset)}>Thêm vào tài nguyên</Button>
            </div>
          </div>
        ) : null}
      </Drawer>
...
function LibraryCard({ asset, onOpen, onAdd }: { asset: AssetLibraryItem; onOpen: () => void; onAdd: () => void }) {
  ...
  return (
    <Card hoverable className="overflow-hidden" styles={{ body: { padding: 0 } }} cover={<button type="button" className="block w-full text-left" onClick={onOpen}>{cover ? <img src={cover} alt={asset.title} className="aspect-[4/3] w-full object-cover" /> : <div className="flex aspect-[4/3] items-center justify-center bg-stone-100 p-5 text-center text-sm leading-6 text-stone-600 dark:bg-stone-900 dark:text-stone-300">{asset.content || "Chưa có ảnh bìa"}</div>}</button>}>
      <button type="button" className="block w-full text-left" onClick={onOpen}>
        <div className="p-4">
          <div className="flex items-start justify-between gap-3">
            <h2 className="line-clamp-1 text-sm font-semibold text-stone-950 dark:text-stone-100">{asset.title}</h2>
            <Tag className="m-0 shrink-0 text-[11px]">{asset.type === "image" ? "Hình ảnh" : "Văn bản"}</Tag>
          </div>
      ...
      <div className="flex items-center gap-2 px-4 pb-4">
        ...
        <Button size="small" icon={<FolderPlus className="size-3.5" />} onClick={onAdd}>Thêm vào tài nguyên</Button>
      </div>
    </Card>
  );
}
